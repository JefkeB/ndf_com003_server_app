...
//
//
int RS232::Open(const std::string devicePath, int baudrate, const char * mode)
{
    int baudr;
    int status;
    ...
    {
      case 'N':
      case 'n': cpar = IGNPAR; break;
      case 'E':
      case 'e': cpar = PARENB; break;
      case 'O':
      case 'o': cpar = (PARENB | PARODD); break;
    ...
    */
    ...
    cport = open(devicePath.c_str(), O_RDWR | O_NOCTTY | O_NDELAY);
    if(cport == -1)
    {
    ...
    int error;
/*
    int error = tcgetattr(cport, &old_port_settings);
    if(error==-1)
    {
      close(cport);
...
//
//
int RS232::Poll(uint8_t * buffer, int size)
{
  return read( cport, buffer, size );
}
...
//
//
int RS232::SendBuf(uint8_t * buffer, int size)
{
  return( write( cport, buffer, size ) );
}
...
//
//
int RS232::SendByte(uint8_t val)
{
  if( write( cport, &val, 1 ) == -1 )
  {
    return 1;
  }
...
*/
  ...
  //Cport[comport_number] = open(comports[comport_number], O_RDWR | O_NOCTTY | O_NDELAY);
  Cport[comport_number] = open(devicePath.c_str(), O_RDWR | O_NOCTTY | O_NDELAY);
